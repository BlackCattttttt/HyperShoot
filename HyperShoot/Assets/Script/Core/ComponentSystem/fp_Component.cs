...
public class fp_Component : MonoBehaviour
{
    ...
    public fp_EventHandler EventHandler
    {
        get
        {
            if (m_EventHandler == null)
                m_EventHandler = (fp_EventHandler)Transform.root.GetComponentInChildren(typeof(fp_EventHandler));
	...
	protected fp_State m_DefaultState = null;
	...
	protected fp_Timer.Handle m_DeactivationTimer = new fp_Timer.Handle();
	...
	protected fp_State m_InitialState = null;       // used at editor runtime only
	public fp_State InitialState { get { return m_InitialState; } }
#endif

	public fp_StateManager StateManager
	{
		get
		{
			if (m_StateManager == null)
				m_StateManager = new fp_StateManager(this, States);
			return m_StateManager;
		}
	}
	public fp_State DefaultState { get { return m_DefaultState; } }
	...
	public void SetState(string state, bool enabled = true, bool recursive = false, bool includeDisabled = false)
	{
		...
		if (recursive)
		{
			for (int c = 0; c < Children.Count; c++)
			{
				if (Children[c] == null)
					continue;
				if ((!includeDisabled) && !(fp_Utility.IsActive(Children[c].gameObject) && Children[c].enabled))
					continue;
	...
	public void ActivateGameObject(bool setActive = true)
	{
		...
		if (setActive)
		{
			...
			foreach (fp_Component s in Siblings)
			{
				s.Activate();
			}
	...
	public void RefreshDefaultState()
	{
		...
		fp_State defaultState = null;

		if (States.Count == 0)
		{
			// there are no states, so create default state
			defaultState = new fp_State(Type.Name, "Default", null);
        ...
        if (defaultState.Preset == null || defaultState.Preset.ComponentType == null)
            defaultState.Preset = new fp_ComponentPreset();
	...
	public void RefreshInitialState()
	{
		...
		m_InitialState = null;
		m_InitialState = new fp_State(Type.Name, "Internal_Initial", null);
		m_InitialState.Preset = new fp_ComponentPreset();
	...
	// settings
	// </summary>
	public void ApplyPreset(fp_ComponentPreset preset)
    {
        fp_ComponentPreset.Apply(this, preset);
        RefreshDefaultState();
        Refresh();
    }
    ...
    /// </summary>
    public fp_ComponentPreset Load(string path)
    {
        fp_ComponentPreset preset = fp_ComponentPreset.LoadFromResources(this, path);
        RefreshDefaultState();
        Refresh();
        return preset;
    }
    ...
    /// and refresh settings
    /// </summary>
    public fp_ComponentPreset Load(TextAsset asset)
    {
        fp_ComponentPreset preset = fp_ComponentPreset.LoadFromTextAsset(this, asset);
        RefreshDefaultState();
        Refresh();
        return preset;
    }
    ...
    public void CacheChildren()
	{
		...
		Children.Clear();
		foreach (fp_Component c in GetComponentsInChildren<fp_Component>(true))
		{
	...
	public void CacheSiblings()
	{
		...
		Siblings.Clear();
		foreach (fp_Component c in GetComponents<fp_Component>())
		{
			if (c != this)
				Siblings.Add(c);
		}
	...
	public void CacheFamily()
	{
		...
		Family.Clear();
		foreach (fp_Component c in transform.root.GetComponentsInChildren<fp_Component>(true))
		{
			if (c != this)
				Family.Add(c);
		}
	...
	protected void VerifyRenderers()
	{
		...
		if ((Renderers[0] == null) || !fp_Utility.IsDescendant(Renderers[0].transform, Transform))
		{
			Renderers.Clear();
			CacheRenderers();
		}
