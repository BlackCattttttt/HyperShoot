...
public class fp_Timer : MonoBehaviour
{
	...
	private void Update()
	{
		...
		m_EventBatch = 0;
		while ((fp_Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
		{
			...
			if (m_EventIterator < 0)
			{
				...
				m_EventIterator = fp_Timer.m_Active.Count - 1;
				break;
			}

			// prevent index out of bounds
			if (m_EventIterator > fp_Timer.m_Active.Count - 1)
				m_EventIterator = fp_Timer.m_Active.Count - 1;

			// execute all due events
			if (Time.time >= fp_Timer.m_Active[m_EventIterator].DueTime ||	// time is up
				fp_Timer.m_Active[m_EventIterator].Id == 0)					// event has been canceled ('Execute' will kill it)
				fp_Timer.m_Active[m_EventIterator].Execute();
			else
			{
				// handle pausing
				if (fp_Timer.m_Active[m_EventIterator].Paused)
					fp_Timer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
				else
					// log lifetime
					fp_Timer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
			}
	...
	private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
	{
		...
		if (m_MainObject == null)
		{
			...
			if (!m_AppQuitting)
			{
				...
				m_MainObject.AddComponent<fp_Timer>();
		...
		fp_Timer.m_EventCount++;
		m_NewEvent.Id = fp_Timer.m_EventCount;
		...
		fp_Timer.m_Active.Add(m_NewEvent);
	...
	/// </summary>
	private static void Cancel(fp_Timer.Handle handle)
	{
	...
	public static void CancelAll()
	{
		...
		for (int t = fp_Timer.m_Active.Count - 1; t > -1; t--)
		{
			fp_Timer.m_Active[t].Id = 0;
		}
	...
	public static void CancelAll(string methodName)
	{
		...
		for (int t = fp_Timer.m_Active.Count - 1; t > -1; t--)
		{
			if (fp_Timer.m_Active[t].MethodName == methodName)
				fp_Timer.m_Active[t].Id = 0;
		}
	...
	{
		fp_Timer.m_Active.Clear();
		fp_Timer.m_Pool.Clear();
...
#endif
	{
		...
		for (int t = fp_Timer.m_Active.Count - 1; t > -1; t--)
		{
			if (fp_Timer.m_Active[t].CancelOnLoad)
				fp_Timer.m_Active[t].Id = 0;
		}
	...
	private class Event
	{
		...
		private void Recycle()
		{
			...
			if (fp_Timer.m_Active.Remove(this))
				m_Pool.Add(this);
		...
		private void Destroy()
		{
			...
			fp_Timer.m_Active.Remove(this);
			fp_Timer.m_Pool.Remove(this);
	...
	public class Handle
	{
		...
		private fp_Timer.Event m_Event = null;	// timer we're pointing at
		...
		public int Id
		{
			...
			set
			{
				...
				for (int t = fp_Timer.m_Active.Count - 1; t > -1; t--)
				{
					if (fp_Timer.m_Active[t].Id == m_Id)
					{
						m_Event = fp_Timer.m_Active[t];
						break;
					}
				}
				if (m_Event == null)
