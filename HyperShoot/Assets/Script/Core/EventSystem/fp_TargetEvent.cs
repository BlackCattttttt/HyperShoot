...
//						fp_TargetEvent.Register(this, "Hello", MyMethod);				// no arguments, no return value
//						fp_TargetEvent<int, string>.Register(this, "Hello", MyMethod);	// 2 arguments, no return value
//						fp_TargetEventReturn<int, string, float, bool>.Register(this, "Hello", MyMethod);	// 3 arguments + return value
//
//					triggering the event from another object:
//						fp_TargetEvent.Send(someObject, "Hello");						// no arguments, no return value
//						fp_TargetEvent<int, string>.Send(someObject, "Hello", 242, "blabla");	// 2 arguments, no return value
//						bool result = fp_TargetEventReturn<int, bool>.Send(someObject, "Hello", 242);	// 1 argument + return value
//
//					FURTHER NOTES:
//
...
//
//					2) 'fp_GlobalEventHandler.UnregisterAll' will unregister all the events registered
...
//					by default). 
//
//					4) setting the optional 'fp_TargetEventOptions' parameter to 'RequireReceiver'
...
public enum fp_TargetEventOptions
{
...
internal static class fp_TargetEventHandler
{
	...
	/// use 'fp_TargetEvent.Register' or 'fp_TargetEventReturn.Register'.
	...
	public static void Register(object target, string eventName, Delegate callback, int dictionary)
	{
		...
		if (target == null) { Debug.LogWarning("Warning: (" + fp_Utility.GetErrorLocation(2) + " -> fp_TargetEvent.Register) Target object is null."); return; }
		if (string.IsNullOrEmpty(eventName)) { Debug.LogWarning("Warning: (" + fp_Utility.GetErrorLocation(2) + " -> fp_TargetEvent.Register) Event name is null or empty."); return; }
		if (callback == null) { Debug.LogWarning("Warning: (" + fp_Utility.GetErrorLocation(2) + " -> fp_TargetEvent.Register) Callback is null."); return; }
		if (callback.Method.Name.StartsWith("<")) { Debug.LogWarning("Warning: (" + fp_Utility.GetErrorLocation(2) + " -> fp_TargetEvent.Register) Target events can only be registered to declared methods."); return; }
	...
	/// events, use 'fp_TargetEvent&lt&gt.Unregister' or
	/// 'fp_TargetEventReturn&lt&gt.Unregister'. To unregister all events,
	/// use 'fp_TargetEventHandler.UnregisterAll'. (this method unregisters
	...
	/// instead use 'fp_TargetEvent.Register' or 'fp_TargetEventReturn.Register'.
	/// (this method looks for an event name registered to the target object and
	/// - if found - returns the delegate.)
	/// </summary>
	public static Delegate GetCallback(object target, string eventName, bool upwards, int d, fp_TargetEventOptions options)
	{
#if UNITY_EDITOR
		if (string.IsNullOrEmpty(eventName))
		{
			Debug.LogError("Error: (" + fp_Utility.GetErrorLocation(2) + ") fp_TargetEvent.Send: Name is null or empty.");
			return null;
		}
#endif
	...
	retry:
		...
		if (!((options & fp_TargetEventOptions.IncludeInactive) == fp_TargetEventOptions.IncludeInactive))
		{
			...
			else
			{
				...
				if (component != null)
				{
					if (!component.enabled || !fp_Utility.IsActive(component.gameObject))
					{
						if (upwards)
							goto recursive;
						return null;
	...
	recursive:
		...
		if ((callback == null) && upwards)
		{
			...
			target = fp_Utility.GetParent(target as Component);
	...
	/// </summary>
	public static void OnNoReceiver(string eventName, fp_TargetEventOptions options)
	{
		if (!((options & fp_TargetEventOptions.RequireReceiver) == fp_TargetEventOptions.RequireReceiver))
			return;
		Debug.LogError("Error: (" + fp_Utility.GetErrorLocation(2) + ") fp_TargetEvent '" + eventName + "' has no receiver!");
	}
					...
					foreach (string eventName in delegates.Keys)
					{
						...
						if (delegates.TryGetValue(eventName, out d))
						{
							...
							foreach (Delegate dd in d.GetInvocationList())
							{
								...
								{
									p += fp_Utility.GetTypeAlias(i.ParameterType) + " " + i.Name + ", ";
								}
								if (p.Length > 0)
									p = p.Remove(p.LastIndexOf(", "));

								s += fp_Utility.GetTypeAlias(dd.Method.ReturnType) + " ";
...
/// (no arguments).
/// </summary>
public static class fp_TargetEvent
{
	...
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 0);
	}
	...
	{
		fp_TargetEventHandler.Unregister(target, eventName, callback);
	}
	public static void Unregister(object target)
	{
		fp_TargetEventHandler.Unregister(target, null, null);
	}

	public static void Unregister(Component component)
	{
		fp_TargetEventHandler.Unregister(component);
	}
	...
	/// 'fp_TargetEventOptions.RequireReceiver' there will be an error if
	/// no such callback can be found.
	/// </summary>
	public static void Send(object target, string eventName, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 0, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
		}
	...
	/// component.
	/// </summary>
	public static void SendUpwards(Component target, string eventName, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 0, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
...
/// (1 generic argument).
/// </summary>
public static class fp_TargetEvent<T>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEvent&ltfloat&gt.Register'
	/// </summary>
	public static void Register(object target, string eventName, Action<T> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 1);
	}
	...
	/// 'fp_TargetEventOptions.RequireReceiver' there will be an error if
	/// no such callback can be found.
	/// example signature: 'fp_TargetEvent&lt;int&gt;.Send'
	/// </summary>
	public static void Send(object target, string eventName, T arg, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 1, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
	...
	/// example signature: 'fp_TargetEvent&lt;int&gt;.SendUpwards'. NOTES:
	/// 1) only the first matching callback detected will execute. The event will fail
	/// if the scan reaches the scene root with no match found. 2) it is best to register
	/// _transforms_ and not other types of component for use with 'SendUpwards', since
	...
	/// correct component.
	/// </summary>
	public static void SendUpwards(Component target, string eventName, T arg, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 1, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
...
/// (2 generic arguments).
/// </summary>
public static class fp_TargetEvent<T, U>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEvent&ltstring, string&gt.Register'
	/// </summary>
	public static void Register(object target, string eventName, Action<T, U> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 2);
	}
	...
	/// 'fp_TargetEventOptions.RequireReceiver' there will be an error if
	/// no such callback can be found.
	/// example signature: 'fp_TargetEvent&lt;int, bool&gt;.Send'
	/// </summary>
	public static void Send(object target, string eventName, T arg1, U arg2, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 2, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects.
	/// example signature: 'fp_TargetEvent&lt;int, bool&gt;.SendUpwards'. NOTES:
	/// 1) only the first matching callback detected will execute. The event will fail
	/// if the scan reaches the scene root with no match found. 2) it is best to register
	/// _transforms_ and not other types of component for use with 'SendUpwards', since
	/// components will be ignored unless the event is sent specifically to the
	/// correct component.
	/// </summary>
	public static void SendUpwards(Component target, string eventName, T arg1, U arg2, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 2, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
...
/// (3 generic arguments).
/// </summary>
public static class fp_TargetEvent<T, U, V>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEvent&ltint, bool, float&gt.Register'
	/// </summary>
	public static void Register(object target, string eventName, Action<T, U, V> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 3);
	}
	...
	/// 'fp_TargetEventOptions.RequireReceiver' there will be an error if
	/// no such callback can be found.
	/// example signature: 'fp_TargetEvent&lt;int, bool, float&gt;.Send'
	/// </summary>
	public static void Send(object target, string eventName, T arg1, U arg2, V arg3, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 3, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects.
	/// example signature: 'fp_TargetEvent&lt;int, bool, float&gt;.SendUpwards'. NOTES:
	/// 1) only the first matching callback detected will execute. The event will fail
	/// if the scan reaches the scene root with no match found. 2) it is best to register
	/// _transforms_ and not other types of component for use with 'SendUpwards', since
	/// components will be ignored unless the event is sent specifically to the
	/// correct component.
	/// </summary>
	public static void SendUpwards(Component target, string eventName, T arg1, U arg2, V arg3, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 3, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return;
...
/// </summary>
public static class fp_TargetEventReturn<R>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEventReturn&ltbool&gt.Register' (where the
	/// type represents the return value).
	/// </summary>
	public static void Register(object target, string eventName, Func<R> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 4);
	}
	...
	/// 'options' is set to 'fp_TargetEventOptions.RequireReceiver' there
	/// will be an error if no such callback can be found.
	/// example signature: 'fp_TargetEventReturn&ltbool&gt.Send' (where the
	/// type represents the return value).
	/// </summary>
	public static R Send(object target, string eventName, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 4, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects. 
	/// example signature: 'fp_TargetEventReturn&ltbool&gt.SendUpwards' (where the
	...
	/// </summary>
	public static R SendUpwards(Component target, string eventName, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 4, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
...
/// </summary>
public static class fp_TargetEventReturn<T, R>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEventReturn&ltfloat, bool&gt.Register' (where
	/// the last type represents the return value).
	/// </summary>
	public static void Register(object target, string eventName, Func<T, R> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 5);
	}
	...
	/// 'options' is set to 'fp_TargetEventOptions.RequireReceiver' there
	/// will be an error if no such callback can be found.
	/// example signature: 'fp_TargetEventReturn&ltfloat, bool&gt.Send' (where
	/// the last type represents the return value).
	/// </summary>
	public static R Send(object target, string eventName, T arg, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 5, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects.
	/// example signature: 'fp_TargetEventReturn&ltfloat, bool&gt.SendUpwards' (where
	/// the last type represents the return value).
	...
	/// </summary>
	public static R SendUpwards(Component target, string eventName, T arg, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 5, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
...
/// </summary>
public static class fp_TargetEventReturn<T, U, R>
{
	...
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEventReturn&lt;string, int, bool&gt;.Register'
	...
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 6);
	}
	...
	/// 'options' is set to 'fp_TargetEventOptions.RequireReceiver' there
	/// will be an error if no such callback can be found.
	/// example signature: 'fp_TargetEventReturn&lt;string, int, bool&gt;.Send'
	/// (where the last type represents the return value).
	/// </summary>
	public static R Send(object target, string eventName, T arg1, U arg2, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 6, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects.
	/// example signature: 'fp_TargetEventReturn&lt;string, int, bool&gt;.SendUpwards'
	/// (where the last type represents the return value).
	/// NOTES: 1) only the first matching callback detected will execute. The event
	/// will fail if the scan reaches the scene root with no match found. 2) it is
	/// best to register _transforms_ and not other types of component for use with
	...
	/// specifically to the correct component.
	/// </summary>
	public static R SendUpwards(Component target, string eventName, T arg1, U arg2, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 6, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
...
/// </summary>
public static class fp_TargetEventReturn<T, U, V, R>
{

	/// <summary>
	/// registers a callback (method or delegate) with the target object.
	/// the callback can be triggered later by sending the event 'eventName'.
	/// example signature: 'fp_TargetEventReturn&ltfloat, int, string, float&gt.Register'
	/// (where the last type represents the return value).
	/// </summary>
	public static void Register(object target, string eventName, Func<T, U, V, R> callback)
	{
		fp_TargetEventHandler.Register(target, eventName, (Delegate)callback, 7);
	}
	...
	/// 'options' is set to 'fp_TargetEventOptions.RequireReceiver' there
	/// will be an error if no such callback can be found.
	/// example signature: 'fp_TargetEventReturn&ltfloat, int, string, float&gt.Send'
	/// (where the last type represents the return value).
	/// </summary>
	public static R Send(object target, string eventName, T arg1, U arg2, V arg3, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, false, 7, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
	...
	/// then onward to its transform and all ancestor transforms recursively until
	/// a callback registered under 'eventName' is found in any of the objects.
	/// example signature: 'fp_TargetEventReturn&ltfloat, int, string, float&gt.SendUpwards'
	/// (where the last type represents the return value).
	/// NOTES: 1) only the first matching callback detected will execute. The event
	/// will fail if the scan reaches the scene root with no match found. 2) it is
	/// best to register _transforms_ and not other types of component for use with
	...
	/// specifically to the correct component.
	/// </summary>
	public static R SendUpwards(Component target, string eventName, T arg1, U arg2, V arg3, fp_TargetEventOptions options = fp_TargetEventOptions.DontRequireReceiver)
	{
	retry:
		Delegate callback = fp_TargetEventHandler.GetCallback(target, eventName, true, 7, options);
		if (callback == null)
		{
			fp_TargetEventHandler.OnNoReceiver(eventName, options);
			return default(R);
