...
public static class fp_PresetEditorGUIUtility
{

	public static fp_Component RunTimeStateButtonTarget = null;
	...
	private static fp_State m_ShowBlockListFor = null;
	...
	/// been overridden
	/// </summary>
	public static void PersistToggle(fp_ComponentPersister persister)
	{
		...
		persister.Component.Persist = fp_EditorGUIUtility.SmallToggle("Persist Play Mode Changes", persister.Component.Persist);
		if (persister.Component.DefaultState.TextAsset != null && persister.Component.Persist == true)
		{
			...
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Locked", s);
			persister.Component.Persist = false;
		}
	...
	/// </summary>
	public static bool PresetFoldout(bool foldout, fp_Component component)
	{
	...
	/// </summary>
	static private void ShowLoadDialog(fp_Component component)
	{

		string path = Application.dataPath.Replace("\\", "/");
		fp_FileDialog.Create(fp_FileDialog.Mode.Open, "Load Preset", path, delegate(string filename)
		{
			if (!fp_ComponentPreset.Load(component, filename))
				fp_FileDialog.Result = "Failed to load preset '" + fp_FileDialog.ExtractFilenameFromPath(filename) + "'.\n\nIs it the correct component type? (" + component.GetType().ToString() + ")";
			else
	...
	/// </summary>
	static private void ShowSaveDialog(fp_Component component)
	{
		ShowSaveDialog(component, false);
	}
	static private void ShowSaveDialog(fp_Component component, bool showLoadDialogAfterwards)
	{
		string path = Application.dataPath;

		fp_FileDialog.Create(fp_FileDialog.Mode.Save, "Save Preset", path, delegate(string filename)
		{
			fp_FileDialog.Result = fp_ComponentPreset.Save(component, filename);
	...
	/// opens a dialog for saving presets
	/// </summary>
	static private void ShowSaveDifferenceDialog(fp_Component component)
	{
		...
		fp_ComponentPreset diskPreset = null;

		fp_FileDialog.Create(fp_FileDialog.Mode.Save, "Save Tweaks", path, delegate(string filename)
		{
			...
			if (diskPreset == null)
			{
				diskPreset = new fp_ComponentPreset();
				// attempt to load target preset into memory, ignoring
				// load errors in the process
				bool logErrorState = fp_ComponentPreset.LogErrors;
				fp_ComponentPreset.LogErrors = false;
				diskPreset.LoadTextStream(filename);
				fp_ComponentPreset.LogErrors = logErrorState;
			}

			fp_FileDialog.Result = fp_ComponentPreset.SaveDifference(component.InitialState.Preset, component, filename, diskPreset);
	...
	/// </summary>
	public static bool StateFoldout(bool foldout, fp_Component component, List<fp_State> stateList, fp_ComponentPersister persister = null)
	{
		...
		if (foldout)
		{
			...
			{
				fp_State s = stateList[v];
				if (!Application.isPlaying)
				{
					fp_PresetEditorGUIUtility.StateField(s, stateList, component);
				...
				{
					fp_PresetEditorGUIUtility.RunTimeStateField(component, s, stateList);
				}
			}
			...
			if (!Application.isPlaying)
			{
				if (GUILayout.Button("Add State", GUILayout.MinWidth(90), GUILayout.MaxWidth(90)))
				{
					...
					stateList.Add(new fp_State(component.GetType().Name, newStateName, ""));
					component.RefreshDefaultState();
					EditorUtility.SetDirty(component);
				}
			}
			...
			if (persister != null)
				fp_PresetEditorGUIUtility.PersistToggle(persister);
			if (Application.isPlaying)
				GUILayout.EndHorizontal();
	...
	/// but different type
	/// </summary>
	public static void UpdateOldStateTypeNames(fp_Component component)
	{
		...
		for (int v = 0; v < component.States.Count; v++)
		{
			...
			// TODO: remove ?
			if (s.TypeName == "fp_FPSController" ||
				s.TypeName == "fp_FPSCamera" ||
				s.TypeName == "fp_FPSSHooter" ||
				s.TypeName == "fp_FPSWeapon")
			{
				s.TypeName = s.TypeName.Replace("fp_FPS", "fp_FP");
	...
	/// 
	/// </summary>
	static void StateBlockList(fp_Component component, fp_State blocker)
	{
		...
		GUILayout.BeginVertical();
		int e = 0;
		foreach (fp_State blockee in component.States)
		{
	...
	/// </summary>
	public static void RunTimeStateField(fp_Component component, fp_State state, List<fp_State> stateList)
	{
		...
		if (!state.Enabled)
		{
			GUILayout.Space(20);
			GUI.enabled = true;
			GUILayout.Label((stateList.Count - stateList.IndexOf(state) - 1).ToString() + ":", fp_EditorGUIUtility.RightAlignedPathStyle, GUILayout.MinWidth(20), GUILayout.MaxWidth(20));
			GUILayout.BeginHorizontal();
			if (GUILayout.Button(state.Name, fp_EditorGUIUtility.CenteredBoxStyle, GUILayout.MinWidth(90), GUILayout.MaxWidth(90)))
			{
				RunTimeStateButtonTarget = component;
				fp_Component[] compos = component.gameObject.GetComponentsInChildren<fp_Component>();
				foreach (fp_Component c in compos)
				{
		...
		else
		{
			...
			if (GUILayout.Button(state.Name,
				((!state.Blocked) ? fp_EditorGUIUtility.CenteredBoxStyleBold : fp_EditorGUIUtility.CenteredStyleBold)
		...
		{
			if (state.Blocked)
				GUILayout.TextField("(Blocked on " + component.GetType().ToString() + "(" + state.BlockCount + "))", fp_EditorGUIUtility.NoteStyle, GUILayout.MinWidth(100));

			if ((state.TextAsset == null) && (!state.Blocked))
				GUILayout.TextField("(No preset)", fp_EditorGUIUtility.NoteStyle, GUILayout.MinWidth(100));
		}
	...
	/// </summary>
	public static void StateField(fp_State state, List<fp_State> stateList, fp_Component component)
	{
		...
		if (orig != state.Name)
		{
			...
			int collisions = -1;
			foreach (fp_State s in stateList)
			{
			...
			if (state.Name == "Default")
			{
				fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "'Default' is a reserved state name.");
				state.Name = orig;
			}
			else if (state.Name.Length == 0)
			{
				fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "State name can't be empty.");
				state.Name = orig;
			}
			else if (collisions > 0)
			{
				fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "There is already a state named '" + state.Name +"'.\nTIP: If you need a similar state name, begin by adding numbers at the end." );
				state.Name = orig;
			}
			else
				EditorUtility.SetDirty(component);
		...
		if (state.Name == "Default")
		{
			...
			{
				GUI.enabled = false;
				GUILayout.TextField("(Inspector)", fp_EditorGUIUtility.NoteStyle, GUILayout.MinWidth(60));
			}
			else
			{
				GUI.enabled = true;
				if (GUILayout.Button("Unlock", fp_EditorGUIUtility.SmallButtonStyle, GUILayout.MinWidth(30), GUILayout.MinHeight(15)))
				{
		...
		else
		{
			...
			if (GUILayout.Button("^", fp_EditorGUIUtility.SmallButtonStyle, GUILayout.MinWidth(15), GUILayout.MaxWidth(15), GUILayout.MinHeight(15)))
			{
			...
			if (GUILayout.Button("B", fp_EditorGUIUtility.SmallButtonStyle, GUILayout.MinWidth(15), GUILayout.MaxWidth(15), GUILayout.MinHeight(15)))
			{
			...
			if (GUILayout.Button("X", fp_EditorGUIUtility.SmallButtonStyle, GUILayout.MinWidth(15), GUILayout.MaxWidth(15), GUILayout.MinHeight(15)))
			{
				fp_MessageBox.Create(fp_MessageBox.Mode.YesNo, "Confirm", "Are you sure you want to delete the state '" + state.Name + "'?", delegate(fp_MessageBox.Answer answer)
				{
					if (answer == fp_MessageBox.Answer.Yes)
					{
						BackupBlockLists(component);
						stateList.Remove(state);
						RestoreBlockLists(component);
	...
	/// </summary>
	static void BackupBlockLists(fp_Component component)
	{

		m_BlockListBackups = new Dictionary<string, List<string>>();
		foreach (fp_State blocker in component.States)
		{
	...
	/// 
	/// </summary>
	public static string GetStateName(fp_Component component, int id)
	{
		if (id == -1)
			return null;
	...
	/// 
	/// </summary>
	public static int GetStateId(fp_Component component, string state)
	{
	...
	/// 
	/// </summary>
	public static fp_State GetState(fp_Component component, string state)
	{

		foreach (fp_State s in component.States)
		{
			if (s.Name == state)
				return s;
		}
	...
	/// </summary>
	static void RestoreBlockLists(fp_Component component)
	{

		foreach (string s in m_BlockListBackups.Keys)
		{
			fp_State blocker = GetState(component, s);
			if (blocker == null)
				continue;
	...
	/// </summary>
	private static void PresetField(fp_State state)
	{
		...
		if (state.TextAsset != orig)
		{
			if (state.TextAsset != null)
			{
				if ((fp_ComponentPreset.GetFileTypeFromAsset(state.TextAsset) == null ||
				fp_ComponentPreset.GetFileTypeFromAsset(state.TextAsset).Name != state.TypeName))
				{
					fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "Error: The file '" + state.TextAsset.name + " ' does not contain a preset of type '" + state.TypeName + "'.");
	...
	/// </summary>
	public static void DefaultStateField(fp_State state)
	{
		...
		GUILayout.TextField("(editor)", fp_EditorGUIUtility.NoteStyle, GUILayout.MinWidth(100));
