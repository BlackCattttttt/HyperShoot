...
public class fp_EventDump
{
	...
	/// </summary>
	public static string Dump(fp_EventHandler handler, string []eventTypes)
	{
	...
	/// 
	/// </summary>
	private static string DumpEventsOfType(string type, string caption, fp_EventHandler handler)
	{
		...
		foreach (FieldInfo f in handler.GetFields())
		{
			...
			switch(type)
			{
				case "vp_Message":
					...
					{
						fp_Message e = (fp_Message)f.GetValue(handler);
				...
				case "vp_Attempt":
					...
					{
						fp_Event e = (fp_Event)f.GetValue(handler);
