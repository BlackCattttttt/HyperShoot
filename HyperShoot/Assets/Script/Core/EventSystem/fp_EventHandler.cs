...
public abstract class fp_EventHandler: MonoBehaviour
{
	...
	protected Dictionary<string, fp_Event> m_EventsByCallback = new Dictionary<string, fp_Event>();
	protected List<fp_Event> m_Events = new List<fp_Event>();
	...
	/// fp_EventHandler, creates instances of them and stores
	...
	protected void StoreHandlerEvents()
	{
		...
		foreach (FieldInfo i in info)
		{
			...
			if (!m_Events.Contains((fp_Event)e))
				m_Events.Add((fp_Event)e);

			// store each event in a dictionary, once for every available prefix,
			// e.g. 'OnStart_Attack', 'OnStop_Attack' etc.
			foreach (string prefix in ((fp_Event)e).Prefixes.Keys)
			{
				m_EventsByCallback.Add(prefix + i.Name, (fp_Event)e);
			}
	...
	/// up until (but not including) fp_StateEventHandler or
	/// fp_EventHandler
	/// </summary>
	public List<FieldInfo> GetFields()
	{
		...
		do
		{
			...
			if (currentType.BaseType != typeof(fp_StateEventHandler) &&
				currentType.BaseType != typeof(fp_EventHandler))
				nextType = currentType.BaseType;
		}
		while (currentType.BaseType != typeof(fp_StateEventHandler) &&
				currentType.BaseType != typeof(fp_EventHandler) &&
				currentType != null);
	...
	public void Register(object target)
	{
		...
		foreach (MethodInfo m in script.Events)
		{
			...
			//Debug.Log(m.Name + ", index: " + index + ", parameterType: " + fp_Utility.GetTypeAlias(e.GetParameterType(index)) + ", returnType: " + fp_Utility.GetTypeAlias(e.GetReturnType(index)));
	...
	public void Unregister(object target)
	{
		...
		foreach (fp_Event e in m_Events)
		{
	...
	protected bool CompareMethodSignatures(MethodInfo scriptMethod, Type handlerParameterType, Type handlerReturnType)
	{
		...
		{
			Debug.LogError("Error: (" + scriptMethod.DeclaringType + ") Return type (" + fp_Utility.GetTypeAlias(scriptMethod.ReturnType) + ") is not valid for '" + scriptMethod.Name + "'. Return type declared in event handler was: (" + fp_Utility.GetTypeAlias(handlerReturnType) + ").");
			return false;
		}
		...
		if (scriptMethod.GetParameters().Length == 1)
		{
			...
			if (((ParameterInfo)scriptMethod.GetParameters().GetValue(0)).ParameterType != handlerParameterType)
			{
				...
				Debug.LogError("Error: (" + scriptMethod.DeclaringType + ") Parameter type (" + fp_Utility.GetTypeAlias(((ParameterInfo)scriptMethod.GetParameters().GetValue(0)).ParameterType) +	") is not valid for '" + scriptMethod.Name + "'. Parameter type declared in event handler was: (" +	fp_Utility.GetTypeAlias(handlerParameterType) + ").");
				return false;
			}
		}
		else if (scriptMethod.GetParameters().Length == 0)
		{
			...
			{
				Debug.LogError("Error: (" + scriptMethod.DeclaringType + ") Can't register method '" + scriptMethod.Name + "' with 0 parameters. Expected: 1 parameter of type (" + fp_Utility.GetTypeAlias(handlerParameterType) + ").");
				return false;
			}
		}
		else if (scriptMethod.GetParameters().Length > 1)
		{
		    ...
		    Debug.LogError("Error: (" + scriptMethod.DeclaringType + ") Can't register method '" + scriptMethod.Name + "' with " + scriptMethod.GetParameters().Length + " parameters. Max parameter count: 1 of type (" + fp_Utility.GetTypeAlias(handlerParameterType) + ").");
		    return false;
		}
		...
		protected static List<MethodInfo> GetMethods(Type type)
		{
			...
			while (type != null)
			{
				foreach (MethodInfo i in type.GetMethods((BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly)))
				{
					...
					// method bodies, including by fp_Timers)
