...
public class fp_FileDialog : EditorWindow
{
	...
	public static void Create(Mode mode, string caption, string path, Callback callback, string extension)
	{
		...
		{
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "Directory does not exist: '" + path + "'");
		...
		fp_FileDialog dialog = (fp_FileDialog)EditorWindow.GetWindow(typeof(fp_FileDialog), true);
	...
	private void DoOpen()
	{
		...
		{
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "File not found. Check the file name and try again.");
		...
		if (!string.IsNullOrEmpty(Result))
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", Result);
	...
	private void DoSave(bool overwriteExisting)
	{
		...
		if (Filename.Contains("\\") ||
			...
			Filename.Contains("|"))
		{
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "A filename cannot contain any of the following characters: \\ / : * ? \" < > |");
			return;
		}
		...
		if (!overwriteExisting && !string.IsNullOrEmpty(Result))
		{
			...
			fp_MessageBox.Create(fp_MessageBox.Mode.YesNo, "Confirm", Result, delegate(fp_MessageBox.Answer answer)
			{
	...
	private void ExecuteCallback()
	{
		...
		if (m_Callback == null)
		{
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "Failed to perform file operation.");
	...
	private void GoUpOneDir()
	{
		...
		{
			fp_MessageBox.Create(fp_MessageBox.Mode.OK, "Error", "Directory does not exist: '" + upPath + "'");
