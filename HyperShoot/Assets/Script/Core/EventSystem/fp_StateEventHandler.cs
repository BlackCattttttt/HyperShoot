...
public abstract class fp_StateEventHandler : fp_EventHandler
{

	List<fp_Component> m_StateTargets = new List<fp_Component>();
	List<fp_Activity> m_Activities = new List<fp_Activity>();
	...
	/// </summary>
	protected void BindStateToActivity(fp_Activity a)
	{
	...
	/// </summary>
	protected void BindStateToActivityOnStart(fp_Activity a)
	{
		...
		delegate()
		{
			foreach (fp_Component c in m_StateTargets)
			{
	...
	/// the same names found on components down the hierarchy
	/// </summary>
	protected void BindStateToActivityOnStop(fp_Activity a)
	{
	...
	protected void StoreStateTargets()
	{
		...
		{
			if (c.Parent == null
				|| (c.Parent.GetComponent<fp_Component>() == null)
				)
			{
	...
	protected void StoreActivities()
	{
		...
		{
			if ((m_Events[e] is fp_Activity) || (m_Events[e].Type.BaseType == typeof(fp_Activity)))
				m_Activities.Add(m_Events[e] as fp_Activity);
		}
	...
	/// yet, false if not
	/// </summary>
	private bool ActivityInitialized(fp_Activity a)
	{
