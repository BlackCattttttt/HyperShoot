...
public class fp_ComponentPersister
{

	public fp_Component Component = null;	// component to persist
	...
	public void PlayModeCallback()
	{
		...
		if(Component == null)
		{
			Debug.LogError("Error! fp_ComponentPersister: Component is null.");
			return;
		}
	...
	private void PersistRestore()
	{
		...
		foreach (FieldInfo f in Component.GetType().GetFields())
		{
			...
			{
				//Debug.LogError("Warning! fp_ComponentPersister can't persist type '" + f.FieldType.Name.ToString() + "'");
			}
			v++;
		}
