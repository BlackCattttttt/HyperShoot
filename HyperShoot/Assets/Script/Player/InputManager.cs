	...
	public virtual void SetupDefaults(string type = "")
	{
		...
		if (type == "" || type == "Buttons")
		{
			if (ButtonKeys.Count == 0)
			{
				...
				AddButton("Reload", GameManager.Instance.Data.Buttons["Reload"]);
				AddButton("Jump", GameManager.Instance.Data.Buttons["Jump"]);
				AddButton("Crouch", GameManager.Instance.Data.Buttons["Crouch"]);
				AddButton("Run", GameManager.Instance.Data.Buttons["Run"]);
				AddButton("Misson", GameManager.Instance.Data.Buttons["Misson"]);
				AddButton("Accept1", KeyCode.Return);
				AddButton("Accept2", KeyCode.KeypadEnter);
				AddButton("Pause", GameManager.Instance.Data.Buttons["Pause"]);
