...
public class fp_AngleBob : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		...
		if ((BobRate.x != 0.0f) && (BobAmp.x != 0.0f))
			m_Offset.x = fp_MathUtility.Sinus(BobRate.x, BobAmp.x, 0);

		if ((BobRate.y != 0.0f) && (BobAmp.y != 0.0f))
			m_Offset.y = fp_MathUtility.Sinus(BobRate.y, BobAmp.y, 0);

		if ((BobRate.z != 0.0f) && (BobAmp.z != 0.0f))
			m_Offset.z = fp_MathUtility.Sinus(BobRate.z, BobAmp.z, 0);
